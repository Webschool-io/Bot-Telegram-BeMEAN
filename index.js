    ...
    if (!crashdata || msg != crashdata.msg) {
      if (msg.chat.type == 'private') {
        userutils.saveUser({
          ...
          if (err) monitutils.notifyAdmins(bot
            , "Erro ao salvar o user " +
            msg.from.id + " no banco. err: `" + JSON.stringify(err) +
            '`');
        });
      }
    }
            ...
            if (command in commands) {
              ...
              if (argsCount >= command.numParams) {
                ...
                bot.sendMessage(msg.chat.id
                  ...
                  argsCount + "). Número de parâmetros exigidos: " +
                  command.numParams +
                  " :/")
                  .catch(console.log);
              }
            } else {
              if (match[3] || msg.chat.type == 'private') bot.sendMessage(
                msg.chat
                  .id, "Eita, esse comando não existe :/").catch(console.log);
            }
          }
        }
      }
    ...
    if (!globalLock) {
      ...
      if (!crashdata || msg != crashdata.msg) {
        const _services = [{
          member: 'reduce'
          , regex: /\.reduce/
          , fn: (bot, msg, match) => services.evalReduce.execute(bot
            , msg)
          , eval: true
        }, {
          member: 'map'
          ...
          , fn: (bot, msg, match) => services.md5.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'gmaps'
          , regex: /onde\s+(?:fica|está|é|eh)\s*(?:o|a)?\s+([^?]+)\??$/i
          , fn: (bot, msg, match) => services.gmaps.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'mdn'
          , regex: /^js\s+([a-zA-Z])+/i
          , fn: (bot, msg, match) => services.mdn.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'npm'
          , regex: /^npm\s+([a-zA-Z])+/i
          , fn: (bot, msg, match) => services.npm.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'wikipedia'
          , regex: /^(Quem|O que|O q|oq) (é|eh|eah|e|significa|são|sao|foi|foram) ([^?]*)\s?\??/i
          , fn: (bot, msg, match) => services.qualeagiria.execute(bot
            , msg
            , {
              'query': match[3]
            })
          ...
          , fn: (bot, msg, match) => services.saudacao.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'tuamae'
          , regex: /bot.*(burro|idiota|retardado|trou?xa|maconheiro|inútil|fiduma(e|é)gua|z(e|é) r(u|o)ela|ot(á|a)rio|v(i|e)ado)/i
          , fn: (bot, msg, match) => services.tuamae.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'lmgtfy'
          , regex: /^gme\s+([a-zA-Z ])+/i
          , fn: (bot, msg, match) => services.gme.execute(bot, msg
            , match)
          , eval: false
        }, {
          ...
          , regex: /webschool/i
          , fn: (bot, msg, match) => services.stickerWebschool.execute(
            bot
            , msg, match)
          , eval: false
        }, {
          ...
          , fn: (bot, msg, match) => services.stickerBemean.execute(bot
            , msg)
          , eval: false
        }, {
          member: 'omdb'
          , regex: /bot, (?:v?o?c?[e|ê]?)? *(?:j[a|á])? *(?:viu|assist[iu|e]|gost[a|ou]|conhece) *(?:de )? *([^?]+)/i
          , fn: (bot, msg, match) => services.omdb.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'config'
          , regex: /#config +([^ ]+) *([^ ]+)*/i
          , fn: (bot, msg, match) => services.config.execute(bot, msg
            , match)
          , eval: false
        }, {
          member: 'zoeiro'
          ...
          s.get(msg.chat.id, 'services', (err, data) => {
            if (data == 'true' || msg.text.match(/.*config.*/)) {
              if (Array.isArray(match)) {
                  ...
                  if (_services[index].regex.test(msg.text)) {
                    ...
                    security.isSecure(msg, service.eval, (secure) => {
                      if (secure) {
                        ...
                        monitutils.notifyBlacklistedEval(msg
                          , bot
                          , service.member);
                        userutils.isUserBlacklisted(msg.from.id
                          , (err, status) => {
                            if (!status) {
                              userutils.blacklistUser(msg.from
                                .id
                                , 'Eval malicioso: `' +
                                msg.text +
                                '`', (err) => {
                ...
                } else if (!recognized && (msg.chat.type == 'group' ||
                  msg.chat.type == 'supergroup') && msg.text) {
                      ...
                      s.get(msg.chat.id, 'learn_local', (err) => {
                        ...
                        if (!hasLink) {
                          treta.insert({
                            ...
                            if (err) monitutils.notifyAdmins(
                              bot
...
const saveCrash = (err) => {
  ...
  monitutils.notifyAdmins(bot
    , `Erro ao processar: \`\`\`${JSON.stringify(processing)}\`\`\`\nErro: \`\`\`${err}\`\`\` `
  )
  process.exit(1);
}
